...
async def SendAchievement(chatId: int, userId: int, achievementsId: list[int]):
    for i in achievementsId:
        ...
        await bot.send_photo(chat_id=chatId,
                             caption=f'<b>{player.name}</b> зарабатывает достижение:\n<b>{achievement.name}</b>\n{achievement.description}',
...
async def GetAchievements(message: types.Message):
    ...
    if len(userAchiv) > 1:
        buttons.append(
            types.InlineKeyboardButton(text='>', callback_data=f'achiv:{message.chat.id}_{message.from_user.id}_1'))
    else:
...
async def GetAchievementsPages(call: types.CallbackQuery):
    ...
    if (page - 1 < 0):
        ...
        buttons.append(types.InlineKeyboardButton(text='<',
                                                  callback_data=f'achiv:{call.message.chat.id}_{call.from_user.id}_{page - 1}'))
    ...
    keyboard = types.InlineKeyboardMarkup()
    keyboard.row(*buttons)
    media = types.input_media.InputMediaPhoto(media=types.InputFile(ROOT / 'static/achiv/' / achievement.image),
                                              caption=replytext, parse_mode='HTML')
    await call.message.edit_media(media, reply_markup=keyboard)


def register_handlers_achievement(dp: Dispatcher):
    dp.register_message_handler(GetAchievements, commands='achievements')
